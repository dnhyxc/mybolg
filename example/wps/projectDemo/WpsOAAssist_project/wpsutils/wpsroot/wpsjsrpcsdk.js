(function (global, factory) {
  ...
  function WpsStart(options) {
    ...
    const baseData = encode(strData)
    const url = `${options.urlBase}/${options.clientType}/runParams`
  ...
  function InitWpsCloudSvr() {
    if (serverId == undefined) {
      ...
      window.location.href = `${'ksoWPSCloudSvr://start=RelayHttpServer' + '&serverId='}${serverId}`
    } // 是否启动wps弹框
  }
  ...
  function codeToBlob(code) {
    const blob = new Blob([code], { type: 'text/javascript' }) // 生成js文件对象
    const objectURL = window.URL.createObjectURL(blob) // 生成js文件的url
    return objectURL
  }

  const RegWebNotifyMap = { wps: {}, wpp: {}, et: {} }
  let bWebNotifyUseTimeout = false
  function WebNotifyUseTimeout(value) {
    bWebNotifyUseTimeout = !!value
  }
  const g_businessId = Number(
    Math.random().toString().substr(3, 5) + Date.parse(new Date())
  ).toString(36)
  let HeartBeatWorker
  if (window.Worker) {
    try {
  ...
  function RegWebNotify(clientType, name, callback, wpsclient) {
    ...
    if (wpsclient) {
      ...
      const RegWebNotifyID = `${new Date().valueOf()}`
      paramStr = {
        id: RegWebNotifyID,
        name,
        type: clientType,
  ...
  function WpsClient(clientType) {
    ...
    this.CloseSilentClient = function (name, callback) {
      ...
      function closeSilentClient(res) {
        if (res.status == 0) this.wpsclient.clientId = ''
  ...
  function IsClientRunning(clientType, callback, wpsclient) {
    const url = `${GetUrlBase()}/isRunning`
    let wrapper = {
  ...
  function WpsAddonGetAllConfig(callBack) {
    const baseData = JSON.stringify({ serverId })
    startWps({
  ...
  function WpsAddonVerifyStatus(element, callBack) {
    const xmlReq = getHttpObj()
    const offline = element.online === 'false'
    const url = offline ? element.url : `${element.url}ribbon.xml`
    ...
    const sendData = FormatSendData(data)
  ...
  function WpsAddonHandleEx(element, cmd, callBack) {
    const data = FormatData(element, cmd)
    startWps({
  ...
  function FormatData(element, cmd) {
    const data = {
      cmd, // "enable", 启用， "disable", 禁用, "disableall", 禁用所有
      name: element.name,
      url: element.url,
  ...
  function FormatSendData(data) {
    ...
    if (serverVersion >= '1.0.2' && serverId != undefined) {
      const base64Data = encode(strData)
