...
function pOpenFile(doc, params, isOnlineDoc) {
    ...
    var downloadParams = GetDocParamsValue(doc, constStrEnum.downloadParams) || {}
 ...
 */
 function InsertPresetTemplateDoc(doc) { // 插入正文模板
  if (!doc) {
      alert('文档不存在!');
      return;
  }
  ...
  return
    ...
    $.ajax({
        ...
        success: function(res) {
            ...
            var data = res.data.bodyFiles || [];
            let Bookmarks = template.Bookmarks;

            data.forEach(function(it) {

              it.fileInfo = JSON.parse(it.fileInfo)
              var bookmark = Bookmarks.Item(it.fileInfo.name);
              ...
              let start = template.Range().End
                ...
                // selections.WholeStory()
                // selections.Range.InsertFile(it.fileInfo.fileUrl + '&filename=' + it.fileInfo.name)
            })
        }
    });
}
...
function pSetOADocumentFlag(doc, params) {
    ...
    console.log('打开后的文档ID', doc.DocID)
    if (doc) {
...
function handleFileAndUpload(suffix, doc, uploadPath, FieldName, saveType = 1) {
    ...
    switch (suffix.toLocaleLowerCase()) {
        case '.pdf':
            ...
            UploadFile(l_strChangeFileName, l_strPath, uploadPath, l_FieldName, OnChangeSuffixUploadSuccess, OnChangeSuffixUploadFail, saveType);
            break;
        case '.uof':
...
function pInsertRInedHead(doc, strFile, bookmarkStart, bookmarkEnd) {
    ...
    if (bookMarks.Exists(bookmarkStart) && bookMarks.Exists(bookmarkEnd)) {
      ...
      alert("套红头失败，您选择的红头模板没有对应书签：" + bookmarkStart + ', ' + bookmarkEnd);
    }
 ...
 *      'bkInsertFile':'' ,正文书签
 */
function InsertRedHeadDoc(doc, callback) { //插入红头
    if (!doc) {
        alert('文档不存在!');
        return;
    }
