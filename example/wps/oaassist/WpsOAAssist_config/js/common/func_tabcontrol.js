...
function pInitParameters() {
  ...
  wps.PluginStorage.setItem(constStrEnum.CloseConfirmTip, true) // 弹出关闭钱提示保存信息后的提示信息
}
 ...
 */
function pDoChangeToOtherDocFormat(p_Doc, p_Suffix, pShowPrompt, p_ShowRevision, saveType = 1) {
  if (!p_Doc) {
    return
  }
  ...
  if (pShowPrompt == true) {
    if (
      !wps.confirm('当前文档将另存一份' + l_suffix + ' 格式的副本，并上传到系统后台，请确认 ？')
    ) {
      return
    }
  }
...
function OnBtnSaveToServer(saveType = 1) {
  ...
  //   pDoChangeToOtherDocFormat(l_doc, l_suffix, false, true)
  // }
  return
}
  ...
  Promise.resolve()
    .then(() => {
      ...
      file = Object.assign({}, file, {
        originalUrl: fileURLObj.redHeadOriginalUrl || fileURLObj.noRedHeadOriginalUrl,
...
function OnUploadToServerSuccess(resp, saveType = 1) {
  ...
  switch (saveType) {
    ...
    case SAVE_TYPE.ORGINAL_NO_RED_HEAD: {
      ...
      wps.PluginStorage.setItem(constStrEnum.SaveAllTemp, JSON.stringify(fileURLObj))
      ...
      //调用转pdf格式函数，强制关闭转换修订痕迹，不弹出用户确认的对话框
      pDoChangeToOtherDocFormat(l_doc, l_suffix, false, true, SAVE_TYPE.PDF_NO_RED_HEAD)
      return;
    }
    ...
    case SAVE_TYPE.ORGINAL_RED_HEAD: {
      ...
      //调用转pdf格式函数，强制关闭转换修订痕迹，不弹出用户确认的对话框
      pDoChangeToOtherDocFormat(l_doc, l_suffix, false, true, SAVE_TYPE.PDF_RED_HEAD)
      return
    }
...
function DoInsertWaterToDoc(watermarkText = '') {
  ...
  try {
    ...
    for (var i = 1; i <= pageCount; i++) {
      ...
      app.ActiveWindow.ActivePane.View.SeekView = 9; // 页眉
      ...
      app.ActiveDocument.Sections.Item(1)
        .Headers.Item(1)
        .Shapes.AddTextEffect(0, watermarkText, '华文新魏', 36, false, false, 0, 0)
        .Select()
...
function OnGetEnabled(control) {
  ...
  switch (eleId) {
    ...
    case 'TabDeveloper': // case "TabPageLayoutWord": //WPS自身tab：页面布局 //WPS自身tab：开发工具
      return true
